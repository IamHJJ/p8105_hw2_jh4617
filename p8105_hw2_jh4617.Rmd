...
There are `r nrow(trash_df)` observations in resulting data, including key variables `r names(trash_df)`. 
...
`r gwynnda_trash_filtered =
  ...
  `r gwynnda_trash |> filter(year == "2021", month == "July") |> sum(cigarette_butts)`
...
Of the recruited participants, `r MCI_baseline |> filter(!is.na(age_at_onset)) |> nrow()`.

The average baseline age is `r mean(pull(MCI_baseline, Age_at_study_baseline))`

There are `r ` of women in the study are APOE4 carriers.
