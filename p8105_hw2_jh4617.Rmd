...
The `inner` dataset is dataset that have merged the `amyloid` and `MCI_baseline` using inner_join function. There are `r nrow(inner)` observations and `r ncol(inner)` variables appear in both baseline and amyloid datafile, including key variables " `r names(inner)`", which tells us about the amyloid ratio in each visit. 
